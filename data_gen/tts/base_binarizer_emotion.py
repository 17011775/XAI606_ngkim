...
class EmotionBinarizer:
    def __init__(self, processed_data_dir=None):
        ...
        self.item_names = []
        self.num_list = []
        self._train_item_names = None
        self._valid_item_names = None
        self._test_item_names = None
        self._test_valid_item_names = None
    ...
    def load_meta_data(self):
        for ds_id, processed_data_dir in enumerate(self.processed_data_dirs):
            ...
            for r_idx, r in tqdm(self.meta_df.iterrows(), desc='Loading meta data.'):
                ...
                self.item2spk[item_name] = r.get('spk_name', 'SPK1') if self.binarization_args['with_spk_id'] else 'SPK1'
                if len(self.processed_data_dirs) > 1:
                ...
                self.items[item_name] = {
                    ...
                    'ph': r['ph'],
                    'wav_fn': r['wav_fn'],
                    'spk_id': r.get('spk_name', 'SPK1'),
                    'item_name': r['item_name']
                }
                self.item_names.append(item_name)
         # 스피커별 데이터 개수 계산
        spk_ids = [self.items[item]['spk_id'] for item in self.items]
        unique_spk_ids = sorted(set(spk_ids))
        for spk_id in unique_spk_ids:
            num = spk_ids.count(spk_id)
            self.num_list.append(num)
        self.item_names = sorted(self.item_names)
            
        #print(self.item_names) # 0011~0020 
        # nan 값을 가진 행 찾기
        print("\nNaN 값을 가진 행:")
        nan_rows = self.meta_df[self.meta_df['item_name'].isna()]
        if not nan_rows.empty:
            print(nan_rows)
        else:
            print("NaN 값을 가진 행이 없습니다.")

    @property
    def train_item_names(self):
        if self._train_item_names is None:
            self._train_item_names = [i for i in self.item_names if i not in self.test_valid_item_names]
        return self._train_item_names

    @property
    def test_item_names(self):
        if self._test_item_names is None:
            self._test_item_names = []
            spk_item_indices = {}
            num = 0
            for spk_id, count in enumerate(self.num_list):
                spk_item_indices[spk_id] = (num, num + count)
                num += count

            for spk_id, (start, end) in spk_item_indices.items():
                self._test_item_names += self.item_names[start+20:start+50]
                self._test_item_names += self.item_names[start+370:start+400]
                self._test_item_names += self.item_names[start+720:start+750]
                self._test_item_names += self.item_names[start+1070:start+1100]
                self._test_item_names += self.item_names[start+1420:start+1450]
            print(self._test_item_names)
            print(len(self._test_item_names))
        return self._test_item_names

    @property
    def valid_item_names(self):
        if self._valid_item_names is None:
            ...
            for spk_id, (start, end) in spk_item_indices.items():
                self._valid_item_names += self.item_names[start:start+20]
                self._valid_item_names += self.item_names[start+350:start+370]
                self._valid_item_names += self.item_names[start+700:start+720]
                self._valid_item_names += self.item_names[start+1050:start+1070]
                self._valid_item_names += self.item_names[start+1400:start+1420]

            if self._test_valid_item_names is None:
                self._test_valid_item_names = []
                for spk_id, (start, end) in spk_item_indices.items():
                    self._test_valid_item_names += self.item_names[start:start+50]
                    self._test_valid_item_names += self.item_names[start+350:start+400]
                    self._test_valid_item_names += self.item_names[start+700:start+750]
                    self._test_valid_item_names += self.item_names[start+1050:start+1100]
                    self._test_valid_item_names += self.item_names[start+1400:start+1450]
            print(self._valid_item_names)
            print(len(self._valid_item_names))
        return self._valid_item_names

    @property
    def test_valid_item_names(self):
        if self._test_valid_item_names is None:
            self.valid_item_names  # 이를 호출하여 self._test_valid_item_names을 초기화합니다.
        return self._test_valid_item_names
    ...
    def process_data(self, prefix):
        ...
        #    np.save(f'{data_dir}/{prefix}_mels_mean_std.npy', [np.mean(mels, axis=0), np.std(mels, axis=0)])
