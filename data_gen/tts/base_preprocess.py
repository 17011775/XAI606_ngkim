...
def remove_null_bytes(obj):
        def _remove_null_bytes(item):
            if isinstance(item, str):
                cleaned = item.replace('\u0000', '')
                if cleaned != item:
                    print(f"Null bytes removed from: {item[:30]}...")
                return cleaned
            elif isinstance(item, list):
                return [_remove_null_bytes(i) for i in item]
            elif isinstance(item, dict):
                return {k: _remove_null_bytes(v) for k, v in item.items()}
            else:
                return item
        return _remove_null_bytes(obj)
...
class BasePreprocessor:
    ...
    def process(self):
        ...
        #for d in meta_data:
        #    print(d)
        print("Checking for empty item_names:")
        for i, d in enumerate(meta_data):
            if 'item_name' not in d or not d['item_name']:
                print(f"Empty or missing item_name at index {i}:")
        #print(d)
        ...
        if self.preprocess_args['use_mfa']:
            ...
            with open(f'{processed_dir}/mfa_dict.txt', 'w') as f:
                f.writelines([f'{l}\n' for l in mfa_dict])
        items = [remove_null_bytes(item) for item in items]
    ...
    def preprocess_first_pass(cls, item_name, txt_raw, txt_processor,
        ...
        try:
            ...
            # 파일 경로와 이름 출력 (디버깅)
            #print(f"wav_fn: {wav_fn}")
            #print(f"new_wav_fn: {new_wav_fn}")
             # new_wav_fn 경로가 비어 있는지 확인
            if not new_wav_fn:
                raise ValueError("new_wav_fn 경로가 비어 있습니다.")
    ...
    def process_wav(item_name, wav_fn, processed_dir, wav_processed_tmp, preprocess_args):
        ...
        if len(processors) >= 1:
            ...
            # 심볼릭 링크가 실제 파일 경로를 올바르게 가리키도록 수정
            if os.path.islink(wav_fn):
                real_path = os.readlink(wav_fn)
                if not os.path.isabs(real_path):
                    real_path = os.path.join(os.path.dirname(wav_fn), real_path)
                if not os.path.exists(real_path):
                    raise FileNotFoundError(f"File {real_path} does not exist.")
                wav_fn = real_path
            
            # 원본 파일 존재 여부 확인
            if not os.path.exists(wav_fn):
                raise FileNotFoundError(f"Original file {wav_fn} does not exist.")
            
            # 절대 경로 사용
            wav_fn = os.path.abspath(wav_fn)
    ...
    def build_mfa_inputs(cls, item, mfa_input_dir, mfa_group, wav_processed_tmp, preprocess_args):
        def check_and_replace_null_bytes(input_string, string_name, item_name):
            if '\0' in input_string:
                print(f"Null byte found in {string_name} for item {item_name}")
        ...
        try:
            ...
            item_name = check_and_replace_null_bytes(item_name, 'item_name', item_name)
            #wav_align_fn = check_and_replace_null_bytes(wav_align_fn, 'wav_align_fn', item_name)
            #ph_gb_word = check_and_replace_null_bytes(ph_gb_word, 'ph_gb_word', item_name)
            ...
            ph_gb_word_nosil = check_and_replace_null_bytes(ph_gb_word_nosil, 'ph_gb_word_nosil', item_name)
            ...
            traceback.print_exc()
            print(f"| Error is caught. item_name: {item['item_name']}. Error: {e}")
            return None
