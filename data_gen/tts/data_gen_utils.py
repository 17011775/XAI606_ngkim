...
def get_pitch(wav_data, mel, hparams):
    ...
    #f0 = f0[:len(mel)]
    f0_interp = interp1d(np.arange(len(f0)), f0, kind='linear')
    f0 = f0_interp(np.linspace(0, len(f0) - 1, len(mel)))
...
def get_mel2ph(tg_fn, ph, mel, hparams):
    ...
    assert not np.any(mel2ph == 0), "There are still unassigned frames"
