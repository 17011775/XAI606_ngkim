...
class BaseTTSDataset(BaseDataset):
    def __init__(self, prefix, shuffle=False, test_items=None, test_sizes=None, data_dir=None):
        ...
        # 멜 평균과 표준편차 로드
        #mel_stats = np.load(f'{self.data_dir}/{prefix}_mels_mean_std.npy', allow_pickle=True)
        #self.mel_mean, self.mel_std = mel_stats[0], mel_stats[1]
        if prefix == 'test':
    ...
    def __getitem__(self, index):
        ...
        spec = torch.Tensor(item['mel'])[:max_frames]
         # 정규화 전 통계 출력
        #print(f"Before normalization - Mean: {spec.mean():.4f}, Std: {spec.std():.4f}, Min: {spec.min():.4f}, Max: {spec.max():.4f}")
        # 멜 스펙트로그램 정규화
        #spec = (spec - self.mel_mean) / self.mel_std
        
        # 정규화 후 통계 출력
        #print(f"After normalization - Mean: {spec.mean():.4f}, Std: {spec.std():.4f}, Min: {spec.min():.4f}, Max: {spec.max():.4f}")
